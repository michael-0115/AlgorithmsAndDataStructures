class StringClass: #Create StringClass
    ...
    def linearSearch (self, target_char):
        for items in self.str_data:  #For loop used to compare each item in Python List with target_char by Linear Search
            if items == target_char: 
                return True          #Return True once a character been found the same as targert character
        return False                 #Return False when all items been compared with no character the same as target_char 
